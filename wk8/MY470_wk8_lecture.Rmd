...
- objects generally immutable in R vs eg lists in Python. Everything in R pretty much is immutable. 
- 
...
- double is R's version of a float
...
<br>
- ls() to see all your objects in your current environment
- Remove objects from your current environment using rm()
- functions should be locally scoped in python. Shoudl refer only to things that are defined
  - function should refer only to things that are defined in its input or in the internals of the function
  - ... 
  - unlke in Python, you dont execute an entire script from top to bottom but can do it line by line
  - important 
...
- data frame as a tack of lists, lists have to be of the same length
...
- but be careful with these because characters work a little bit differently than str in Python 
- named vs unnamed list
...
- in R, if you do not add the L at the end, then it will default to double.
...
- c() used to construct a vector and then save that vector as an object.
...
- much less flexible, cannot blend types, could not be 1, 2 and then some complicated data structure
...
- only get hello but not world, because the second index is FALSE. 
...
- when you can, vectorise 
- work with copilot in R 
...
- much more efficient than using a loop in R, the apply functions vs for loops 
- apply function broadly over data structure
...
- in Python classes and methods are the bread and butter of everything you do. 
- in R, this is not really the case, it is functions that are the bread and butter you do
- anytime you want to change data or manipulate data, everyting should be wrapped into functions -> keeps workspace clean
everything properly scoped.
- in R, import your own functions as modules just like in Python
- want to run entire R script and for your workspace to never change, stay entirely clean
- best way: never save anything to your environment other than the functions you are going to use, this is where you want to get to for 
